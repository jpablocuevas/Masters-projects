...
C     = N*sigma^3/V, donde sigma es un diámetro efectivo de particula
C       definido por u(sigma)=0
...
c      DELR=(1.d0-0.75*S)*XL/XHIST
      DELR=(XL-0.75D00)/XHIST
      VOLME=XL*YL*ZL
      CONST=2.0D00/3.0D00*XN*XN*PI*XNAV/VOLME
c      NEND=INT((0.5D00*XC-S)*XHIST/(1.d0-S))
c      NEND=INT((0.5D00*XC-0.75d0*S)*XHIST/(1.d0-0.75d0*S))
      NEND=INT((0.5D00*XC*XL-0.75d0)/DELR)
      DO 1 L=1,NEND
      X=FLOAT(L)
      R=DELR*(X-0.5D00)
      RB=DELR*(X-1.0D00)
